(() => {
  ...
  form?.addEventListener("submit", async (event) => {
    ...
    if (!message) {
      formError.textContent = "Оставь, пожалуйста, пожелание для Маши.";
      form.querySelector("textarea[name='message']")?.focus();
      return;
    }
  ...
  document.querySelectorAll("[data-unlock]").forEach((button) => {
    button.addEventListener("click", async () => {
      const reservationId = button.dataset.unlock;
      if (!reservationId || !window.ADMIN_KEY) return;
      if (!window.confirm("Вернуть этот подарок в список, чтобы его можно было забронировать снова?")) return;
      button.disabled = true;
      try {
        const response = await fetch(`/api/admin/reservations/${encodeURIComponent(reservationId)}/cancel`, {
          method: "POST", headers: { "X-Admin-Key": window.ADMIN_KEY },
        });
        if (!response.ok) throw new Error();
        document.querySelector(`[data-reservation="${reservationId}"]`)?.remove();
      } catch (_) {
        window.alert("Не получилось разблокировать подарок. Обнови страницу и попробуй ещё раз.");
